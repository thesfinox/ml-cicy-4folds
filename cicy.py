...
if __name__ == '__main__':
    ...
    if p['fmap'] is not None:
        x = FeatureMap(p['fmap'], alpha=p['alpha'], seed=p['seed'])(x)
    else:
        x = keras.layers.Flatten()(x)
        
    # dropout
    if p['dropout'] is not None:
        x = keras.layers.Dropout(rate=p['dropout'], seed=p['seed'])(x)
    ...
    if p['dense'] is not None:
        ...
        for u in p['dense']:
            ...
            x = DenseActivation(u, alpha=p['alpha'], l1_reg=p['l1_reg'], l2_reg=p['l2_reg'], norm=p['norm'], dropout=p['full_dropout'], seed=p['seed'])(x)
